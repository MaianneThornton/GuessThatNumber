...
while (restartGame){
  ...
  // Using parseInt to attempt to convert the user's response into a number value. (NOTE: The value returned from a prompt is a string valueAlso, if the value cannot be converted then the value returned will be NaN (Not A Number)).
  ...
  randomNum = Math.floor(Math.random() * rangeNum) + 1; //Runs behind the scenes (not visible to user).
  ...
  if (attempts < rangeNum){
    then prompt(`Please enter a number less than the number of attempts`);
  }
